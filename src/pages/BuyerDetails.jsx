...
export default function BuyerDetails() {
  ...
  const [personal, setPersonal] = useState({ name: "", email: "", mobile: "", dob: "", address: "" });
  const [property, setProperty] = useState({ surveyNumber: "", location: "", size: "" });
  const [docs, setDocs] = useState({ idProof: null, addressProof: null, propertyDocs: null });
  const [docNames, setDocNames] = useState({ idProof: "", addressProof: "", propertyDocs: "" });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Verification is best-effort; submission success should not wait forever.
      try {
        const isVerified = await withTimeout(verifyWithTNREGINET(property.surveyNumber), 8000, "Verify timeout");
        await withTimeout(updateVerificationStatus("buyers", uid, isVerified), 8000, "Status update timeout");
      } catch {
        // keep silent; admin/user can still see submitted data.
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4">
        <Card className="max-w-md w-full p-10 text-center">
          ...
          <p className="text-slate-300 mb-6">Your buyer registration is saved successfully.</p>
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 py-10 px-4">
      <div className="max-w-xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-black text-white mt-2">Buyer Registration</h1>
        </div>
          ...
          <div className="p-8">
            ...
            {step === 1 && (
              <div className="flex flex-col gap-4">
                ...
                <Input label="Property Location" value={property.location} onChange={setProp("location")} />
                <Input label="Property Size (sq. ft)" type="number" value={property.size} onChange={setProp("size")} />
              </div>
            )}

            {step === 2 && (
              <div className="flex flex-col gap-4">
                <FileUpload label="ID Proof" accept=".pdf,.jpg,.jpeg,.png" onChange={handleDoc("idProof")} fileName={docNames.idProof} />
