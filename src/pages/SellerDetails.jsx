...
export default function SellerDetails() {
  ...
  const verifyWithTNREGINET = async (surveyNumber) => {
    return new Promise((resolve) => setTimeout(() => resolve(surveyNumber.toUpperCase().startsWith("TN")), 2000));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const [idUrl, propUrl] = await Promise.all([
        uploadFile(`sellers/${uid}/id_proof_${Date.now()}_${docs.idProof.name}`, docs.idProof),
        uploadFile(`sellers/${uid}/property_docs_${Date.now()}_${docs.propertyDocs.name}`, docs.propertyDocs),
      ]);
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4">
        <Card className="max-w-md w-full p-10 text-center">
          ...
          <p className="text-slate-400 mb-6">
            Your application is submitted. AI analysis is complete and admin verification is pending.
          </p>
          {videoData?.sentiment && (
            <div
            ...
            >
              AI Sentiment: {videoData.sentiment.label.toUpperCase()} | Pending Admin Decision
            </div>
          )}
          ...
          <div className="p-8">
            ...
            {step === 0 && (
              <div className="flex flex-col gap-4">
                ...
                <Input
                  ...
                  placeholder="Sri Lakshmi Properties"
            ...
            {step === 1 && (
              <div className="flex flex-col gap-4">
                ...
                <Input
                  label="Land Ownership Document Number"
                  placeholder="Doc No. / Patta No."
                  value={property.ownershipDocs}
                  onChange={setProp("ownershipDocs")}
                />
              </div>
            )}
            ...
            {step === 2 && (
              <div className="flex flex-col gap-4">
                ...
                <FileUpload
                  label="Property Documents (Patta / Sale Deed)"
                  accept=".pdf,.jpg,.png"
                  onChange={handleDoc("propertyDocs")}
                  fileName={docNames.propertyDocs}
                />
              </div>
            )}
            ...
            {step === 3 && (
              <div className="flex flex-col gap-4">
                <Alert type="info">
                  <strong>Video Consent Required:</strong> Record a short video and submit for AI + admin verification.
                </Alert>
                <VideoConsent onComplete={handleVideoComplete} />
                {videoComplete && (
                  <Button onClick={handleSubmit} loading={loading || verifying} className="w-full mt-2">
                    {verifying ? "Verifying with TNREGINET..." : "Submit Registration for Review"}
                  </Button>
                )}
              </div>
            )}
